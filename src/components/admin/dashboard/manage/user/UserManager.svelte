<script lang="ts">
    ...
    onMount(async () => {
        const { data, error } = await actions.administration.getAllProfiles({ username: "" });
...
<div class="bg-[#111111] border border-white/10 h-screen">
    <div class="p-6 flex flex-col">
        ...
        <div class="border border-white/10 rounded-xl bg-white/2">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-white/5 text-gray-400 uppercase text-xs font-semibold tracking-wide">
                ...
                {:else}
                    {#each profiles as user (user.user_id)}
                        <tr class="border-t border-white/5 hover:bg-white/5 transition-colors">
                            ...
                            <td class="p-4">
                                <span class="px-2 py-1 rounded-md bg-[#00A0DE]/10 text-[#00A0DE] text-xs border border-[#00A0DE]/20">
                                    {user.is_admin ? "Yes" : "No"}
                                </span>
                            </td>
                            <td class="p-4 text-sm text-gray-500">{user.user_id}</td>
                            <td class="p-4 text-right">
                                <button
                                    onclick={() => openEditUserModal(user)}
                                    class="text-[#00A0DE] hover:text-white transition"
                                >
                                    Edit
                                </button>
                                <button
                                    onclick={() => deleteUser(user.user_id)}
                                    class="text-red-400 hover:text-red-300 ml-4 transition"
                                >
                                    Delete
                                </button>
                            </td>
        ...
        <div class="flex justify-between items-center mt-6">
            <p class="text-gray-500 text-sm">Page {currentPage} of {totalPages || 1}</p>
...
<dialog
    ...
    onclick={(e) => { if (e.target === editUserModal) editUserModal.close(); }}
