<script lang="ts">
    ...
    async function createNewSeries(name: string, description: string) {
        const { data, error } = await actions.series.createSeries({ name, description });
...
<button
    ...
    >
    <div class="bg-slate-950 p-6 rounded-[calc(1rem-1px)] h-full flex flex-col justify-center">
            ...
            {:else}
                {#each seriesFiltered as s (s.id)}
                    ...
                    <div class="flex gap-3">
                        <button onclick={() => { editingSeries = s; showEditModal = true; }} class="text-blue-400 text-sm">Edit</button>
                        <button onclick={() => deleteSeries(s.id)} class="text-rose-400 text-sm">Delete</button>
                    </div>
                    </div>
                {/each}
            {/if}
